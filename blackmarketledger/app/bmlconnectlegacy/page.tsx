...
export default function BMLDashboard() {
  ...
  const initiateStorageSwitch = async (target: StorageLocation, label: string) => {
    ...
    // Helper to read transactions for a given storage location
  ...
  const handleMigrationConfirm = async (type: "merge" | "overwrite" | "none") => {
    ...
    try {
      ...
      // If either direction involves the extension, handle migration manually
      if (target === 'extension') {
        if (type !== 'none') {
        ...
        // Reload so the global journal picks up extension preference/data
      ...
      if (source === 'extension' && (target === 'drive' || target === 'browser')) {
        ...
        if (type === 'overwrite') {
          ...
          if (target === 'drive' && driveApiKey) {
            await writeGoogleDriveData(driveApiKey, extData);
          }
        } else if (type === 'merge') {
          ...
          if (target === 'drive' && driveApiKey) {
            await writeGoogleDriveData(driveApiKey, merged);
          }
        }

        // Let the shared switchStorageLocation load the new target data into app state
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-xl border border-border bg-foreground/5 p-5">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className={`rounded-xl border-2 p-4 transition-all ${storageLocation === "extension" ? "border-primary bg-primary/5" : "cursor-pointer border-border bg-background hover:border-primary/50"}`}
                    onClick={() => initiateStorageSwitch("extension", "Extension Database")}
                  >
          ...
          <div className="w-full max-w-lg rounded-3xl bg-panel border border-border overflow-hidden shadow-2xl">
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="pt-2 text-center">
                <button onClick={() => setShowMigrationDialog(false)} className="text-[10px] font-bold text-foreground/30 hover:text-foreground uppercase tracking-widest transition-colors">Discard Switch Request</button>
              </div>
            </div>
          </div>
        </div>
